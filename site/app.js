const TWEAK_DEFAULTS = { accent: "#C9A84C", headline: "perda", marqueeSpeed: 38, showMarquee: !0 },
  HEADLINES = {
    perda: { l1: "VOCÊ PERDE", l2: "CLIENTE", l2gold: !0, l3stroke: "TODO", l3rest: "DIA" },
    zap: { l1: "SEU ZAP", l2: "FICA SEM", l3gold: "NINGUÉM" },
    silencio: { l1: "ELE SUMIU", l2: "E VOCÊ", l3gold: "NEM VIU" },
  },
  PRESETS = {
    classico: { accent: "#C9A84C", headline: "perda", marqueeSpeed: 38, showMarquee: !0 },
    navalha: { accent: "#C0392B", headline: "zap", marqueeSpeed: 34, showMarquee: !0 },
    money: { accent: "#1F8A5B", headline: "perda", marqueeSpeed: 38, showMarquee: !0 },
    premium: { accent: "#6F3FF5", headline: "silencio", marqueeSpeed: 42, showMarquee: !0 },
    street: { accent: "#D9531E", headline: "zap", marqueeSpeed: 30, showMarquee: !0 },
    prata: { accent: "#9FB2C4", headline: "perda", marqueeSpeed: 46, showMarquee: !1 },
  };
function applyPresetFromURL(n) {
  try {
    const e = new URLSearchParams(window.location.search);
    let a = { ...n };
    const o = (e.get("preset") || "").toLowerCase();
    return (
      o && PRESETS[o] && (a = { ...a, ...PRESETS[o] }),
      e.get("accent") && (a.accent = e.get("accent")),
      e.get("headline") && HEADLINES[e.get("headline")] && (a.headline = e.get("headline")),
      e.get("marquee") && (a.marqueeSpeed = Number(e.get("marquee")) || a.marqueeSpeed),
      (e.get("marquee") === "0" || e.get("nomarquee") === "1") && (a.showMarquee = !1),
      a
    );
  } catch {
    return n;
  }
}
window.PRESETS = PRESETS;
function HeroSwap({ accent: n, headline: e }) {
  const a = HEADLINES[e] || HEADLINES.perda;
  return React.createElement(
    "header",
    { className: "hero", id: "top" },
    React.createElement("div", { className: "hero-grid-bg" }),
    React.createElement("div", { className: "hero-noise" }),
    React.createElement(
      "div",
      { className: "hero-inner" },
      React.createElement(
        "div",
        null,
        React.createElement(
          "div",
          { className: "eyebrow hero-eyebrow" },
          "Atendimento operado pra barbearia · Est. 2025",
        ),
        React.createElement(
          "h1",
          null,
          a.l1,
          React.createElement("br", null),
          a.l2gold ? React.createElement("span", { className: "gold" }, a.l2) : a.l2,
          React.createElement("br", null),
          a.l3stroke
            ? React.createElement(
                React.Fragment,
                null,
                React.createElement("span", { className: "stroke" }, a.l3stroke),
                " " + a.l3rest,
              )
            : React.createElement("span", { className: "gold" }, a.l3gold),
        ),
      ),
      React.createElement(
        "div",
        { className: "hero-side" },
        React.createElement(
          "div",
          { className: "hero-tag" },
          "Acontece no zap, enquanto você tá com a mão na cabeça do cliente. A gente atende esse zap por você: responde em segundos, confirma os horários e chama de volta quem sumiu.",
        ),
        React.createElement(
          "div",
          { className: "hero-meta" },
          React.createElement(
            "div",
            null,
            React.createElement("strong", null, "NICHO"),
            "Só barbearia",
          ),
          React.createElement(
            "div",
            null,
            React.createElement("strong", null, "FOCO"),
            "Brasília / DF",
          ),
          React.createElement(
            "div",
            null,
            React.createElement("strong", null, "SETUP"),
            "72 horas",
          ),
          React.createElement(
            "div",
            null,
            React.createElement("strong", null, "GARANTIA"),
            "Primeiro mês",
          ),
        ),
      ),
    ),
    React.createElement(
      "div",
      { className: "hero-bottom" },
      React.createElement(
        "div",
        { className: "hero-scroll" },
        React.createElement("span", { className: "line" }),
        "SCROLL · A CONTA QUE NINGUÉM FAZ",
      ),
      React.createElement(
        "div",
        { className: "hero-cta-row" },
        React.createElement(
          "a",
          { href: "#contato", className: "btn btn-gold" },
          "Ver meu diagnóstico →",
        ),
        React.createElement(
          "a",
          { href: "#sistema", className: "btn btn-ghost" },
          "Ver a conversa rodando",
        ),
      ),
    ),
  );
}
function App() {
  const [n, e] = window.useTweaks(applyPresetFromURL(TWEAK_DEFAULTS));
  return (
    React.useEffect(() => {
      (document.documentElement.style.setProperty("--gold", n.accent),
        document.documentElement.style.setProperty("--gold-light", lighten(n.accent, 0.12)),
        document.documentElement.style.setProperty("--gold-deep", darken(n.accent, 0.18)));
    }, [n.accent]),
    React.useEffect(() => {
      const a = document.querySelector(".marquee-track");
      a && (a.style.animationDuration = `${n.marqueeSpeed}s`);
    }, [n.marqueeSpeed]),
    window.useReveal(),
    React.createElement(
      React.Fragment,
      null,
      React.createElement(window.Nav, null),
      React.createElement(HeroSwap, { accent: n.accent, headline: n.headline }),
      n.showMarquee && React.createElement(window.Marquee, null),
      React.createElement(window.Problem, null),
      React.createElement(window.Absolution, null),
      React.createElement(window.WhyOthersFail, null),
      React.createElement(window.WhatsAppDemo, null),
      React.createElement(window.Agenda, null),
      React.createElement(window.Process, null),
      React.createElement(window.Metrics, null),
      React.createElement(window.Differentiator, null),
      React.createElement(window.Cases, null),
      React.createElement(window.Operators, null),
      React.createElement(window.FAQ, null),
      React.createElement(window.CTA, null),
      React.createElement(window.Footer, null),
      React.createElement(
        window.TweaksPanel,
        null,
        React.createElement(window.TweakSection, { label: "Identidade" }),
        React.createElement(window.TweakColor, {
          label: "Cor de acento",
          value: n.accent,
          options: ["#C9A84C", "#D9531E", "#1F8A5B", "#6F3FF5", "#E8C97A"],
          onChange: (a) => e("accent", a),
        }),
        React.createElement(window.TweakSection, { label: "Hero" }),
        React.createElement(window.TweakSelect, {
          label: "Headline principal",
          value: n.headline,
          options: [
            { value: "perda", label: "Você perde cliente todo dia" },
            { value: "zap", label: "Seu zap fica sem ninguém" },
            { value: "silencio", label: "Ele sumiu e você nem viu" },
          ],
          onChange: (a) => e("headline", a),
        }),
        React.createElement(window.TweakSection, { label: "Marquee" }),
        React.createElement(window.TweakToggle, {
          label: "Mostrar marquee dourado",
          value: n.showMarquee,
          onChange: (a) => e("showMarquee", a),
        }),
        React.createElement(window.TweakSlider, {
          label: "Velocidade",
          value: n.marqueeSpeed,
          min: 15,
          max: 80,
          step: 1,
          unit: "s",
          onChange: (a) => e("marqueeSpeed", a),
        }),
      ),
    )
  );
}
function hexToRgb(n) {
  const e = n.replace("#", "").match(/^([0-9a-f]{2})([0-9a-f]{2})([0-9a-f]{2})$/i);
  return e
    ? { r: parseInt(e[1], 16), g: parseInt(e[2], 16), b: parseInt(e[3], 16) }
    : { r: 201, g: 168, b: 76 };
}
function rgbToHex(n, e, a) {
  const o = (t) =>
    Math.round(Math.max(0, Math.min(255, t)))
      .toString(16)
      .padStart(2, "0");
  return "#" + o(n) + o(e) + o(a);
}
function lighten(n, e) {
  const { r: a, g: o, b: t } = hexToRgb(n);
  return rgbToHex(a + (255 - a) * e, o + (255 - o) * e, t + (255 - t) * e);
}
function darken(n, e) {
  const { r: a, g: o, b: t } = hexToRgb(n);
  return rgbToHex(a * (1 - e), o * (1 - e), t * (1 - e));
}
ReactDOM.createRoot(document.getElementById("root")).render(React.createElement(App, null));
