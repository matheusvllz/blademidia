...
> - Visão semanal em grade estendida pela change `add-agenda-visao-semanal` (concluída
>   2026-07-15): visão adicional (dias × horários), toggle todos/um barbeiro, indicador de
>   ocupação (compartilhado com `relatorios`, ADR-0010) e ação de criar/abrir a partir da grade.
> - Login opcional do barbeiro estendido pela change `add-fidelizacao-e-funcionarios` (Fase 4,
>   concluída 2026-07-16): barbeiro pode ou não ter login (papel `funcionario`, gerido por
>   `auth-tenancy`); esta capability continua dona do recurso `barbers` em si.
>
> Escopo atual: Fase 2 + visão semanal em grade + login opcional do barbeiro. Envio automático de mensagem (confirmação 24h,
> reativação), auto-agendamento pelo cliente final e conversação de IA pertencem a outras
> capabilities (`whatsapp-canal`, `atendimento-ia`, `confirmacao-agendamento`,
> `reativacao-clientes`) e não estão aqui — esta capability expõe a fronteira de domínio que
> elas vão consumir.
...
da agenda) escopados ao `barbershop_id`.
...
#### Scenario: Dia ou semana sem agendamentos
...
de ocupação da capability `relatorios` (ADR-0010).
