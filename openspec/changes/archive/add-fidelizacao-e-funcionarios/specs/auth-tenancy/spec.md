# Capability: auth-tenancy (delta — change `add-fidelizacao-e-funcionarios`)
...
> `openspec/specs/auth-tenancy/spec.md` — primeira vez que esta capability (antes só candidata)
> ganha spec real.
>
> Escopo desta primeira versão: dois papéis (`dono`, `funcionario`), funcionário vinculado a um
> barbeiro do catálogo (`agendamento`). Papéis adicionais, acesso de suporte da Blade e convite
> por e-mail ficam fora (ver Non-Goals do proposal).
